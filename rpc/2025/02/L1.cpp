...
double get_shortest_time(int start, int end, double delta_t, auto &adj) {
    ...
    while(!pq.empty()) {
        ...
        for(edge &e : adj[u]) {
            ...
            if(time[u] < delta_t) {
                time_to_v = e.l / e.v1;
                if(delta_t < time_to_v + time[u]) {
                    double t1 = delta_t - time[u];
                    double d2 = e.l - t1 * e.v1;
                    double t2 = d2 / e.v2;
                    time_to_v = t1 + t2;
                }
            } else {
...
void solve() {
    ...
    double time = get_shortest_time(0, n-1, (double) t, adj);
