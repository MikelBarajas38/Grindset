...
static void __message_loop_Main(int __current_function, int __noreturn) {
	...
	while (readfull(&__in, &__msgid, sizeof(__msgid), 0)) {
		...
		switch (__msgid) {
			case 0x13857295u: {
				// Main -> tesoro.BuscaTesoros
				uint32_t __cookie;
				int BuscaTesoros_n;
				readfull(&__in, &BuscaTesoros_n, sizeof(int), 1);
				int BuscaTesoros_m;
				readfull(&__in, &BuscaTesoros_m, sizeof(int), 1);
				int BuscaTesoros_k;
				readfull(&__in, &BuscaTesoros_k, sizeof(int), 1);
				readfull(&__in, &__cookie, sizeof(__cookie), 1);

				BuscaTesoros(BuscaTesoros_n, BuscaTesoros_m, BuscaTesoros_k);
...
int main(int argc, char* argv[]) {
	...
	openstream(&__in, "./tesoro_in", O_RDONLY);
	openstream(&__out, "./tesoro_out", O_WRONLY);
