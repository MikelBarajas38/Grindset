...
static struct __stream __tesoro_in, __tesoro_out;

void __NOINLINE __libinteractive_init() {
	openstream(&__tesoro_in, "./tesoro_in", O_WRONLY);
	openstream(&__tesoro_out, "./tesoro_out", O_RDONLY);
}
...
static void __message_loop_tesoro(int __current_function, int __noreturn) {
	uint32_t __msgid;
	while (readfull(&__tesoro_out, &__msgid, sizeof(__msgid), 0)) {
		if (__msgid == __current_function) return;
		switch (__msgid) {
			case 0x71651df7u: {
				// tesoro -> Main.Preguntar
				uint32_t __cookie;
				int Preguntar_i;
				readfull(&__tesoro_out, &Preguntar_i, sizeof(int), 1);
				int Preguntar_j;
				readfull(&__tesoro_out, &Preguntar_j, sizeof(int), 1);
				readfull(&__tesoro_out, &__cookie, sizeof(__cookie), 1);

				int result =
				Preguntar(Preguntar_i, Preguntar_j);

				writefull(&__tesoro_in, &__msgid, sizeof(__msgid));
				writefull(&__tesoro_in, &result, sizeof(result));
				writefull(&__tesoro_in, &__cookie, sizeof(__cookie));
				writeflush(&__tesoro_in);
				break;
			}
			...
			case 0x685d93e7u: {
				...
				uint32_t __cookie;
				int Cavar_i;
				readfull(&__tesoro_out, &Cavar_i, sizeof(int), 1);
				int Cavar_j;
				readfull(&__tesoro_out, &Cavar_j, sizeof(int), 1);
				readfull(&__tesoro_out, &__cookie, sizeof(__cookie), 1);

				bool result =
				Cavar(Cavar_i, Cavar_j);
...
void BuscaTesoros(int n, int m, int k) {
	...
	writefull(&__tesoro_in, &__msgid, sizeof(__msgid));
	writefull(&__tesoro_in, &n, sizeof(int));
	writefull(&__tesoro_in, &m, sizeof(int));
	writefull(&__tesoro_in, &k, sizeof(int));
	writefull(&__tesoro_in, &__cookie, sizeof(__cookie));
	writeflush(&__tesoro_in);

	__message_loop_tesoro(__msgid,  0 );
	uint32_t __cookie_result = 0;
	readfull(&__tesoro_out, &__cookie_result, sizeof(__cookie_result), 1);
