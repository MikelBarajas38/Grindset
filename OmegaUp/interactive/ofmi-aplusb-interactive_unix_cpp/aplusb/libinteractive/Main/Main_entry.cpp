...
static struct __stream __aplusb_in, __aplusb_out;

void __NOINLINE __libinteractive_init() {
	openstream(&__aplusb_in, "./aplusb_in", O_WRONLY);
	openstream(&__aplusb_out, "./aplusb_out", O_RDONLY);
}
...
static void __message_loop_aplusb(int __current_function, int __noreturn) {
	uint32_t __msgid;
	while (readfull(&__aplusb_out, &__msgid, sizeof(__msgid), 0)) {
		if (__msgid == __current_function) return;
		switch (__msgid) {
			case 0x71651df7u: {
				// aplusb -> Main.valorB
				uint32_t __cookie;
				readfull(&__aplusb_out, &__cookie, sizeof(__cookie), 1);

				int result =
				valorB();

				writefull(&__aplusb_in, &__msgid, sizeof(__msgid));
				writefull(&__aplusb_in, &result, sizeof(result));
				writefull(&__aplusb_in, &__cookie, sizeof(__cookie));
				writeflush(&__aplusb_in);
				break;
			}
...
int suma(int A) {
	...
	writefull(&__aplusb_in, &__msgid, sizeof(__msgid));
	writefull(&__aplusb_in, &A, sizeof(int));
	writefull(&__aplusb_in, &__cookie, sizeof(__cookie));
	writeflush(&__aplusb_in);

	__message_loop_aplusb(__msgid,  0 );
	int __ans = 0;
	readfull(&__aplusb_out, &__ans, sizeof(__ans), 1);
	uint32_t __cookie_result = 0;
	readfull(&__aplusb_out, &__cookie_result, sizeof(__cookie_result), 1);
